...
const AccountsPage = () => {
  ...
  useEffect(() => {
    const fetchAccountData = async () => {
      try {
        const response = await axios.get(`https://backendcrmnurenai.azurewebsites.net/accounts/${id}`);
  ...
  const contactPersons = [
    { name: 'Jane Smith', position: 'Manager', email: 'jane@example.com', phone: '+1 234-567-8901' },
    { name: 'Mike Johnson', position: 'Sales Rep', email: 'mike@example.com', phone: '+1 345-678-9012' },
  ...
  return (
    <div className="container_">
      ...
      <div className="header">
        ...
        <a href={`mailto:${account.email}`} className="btn btn-primary" style={{float:"right", color:"white"}}>Send Email</a>
        <div className="header-content">
           <h1>{account.company}</h1> 
        </div>
        <a href={account.website} target="_blank" rel="noopener noreferrer">Visit Website</a>
      </div>
      <div className="overview">
        <h2 >Overview</h2>
        <p><strong>Account Owner:</strong> {account.Name}</p>
        ...
        <p><strong>Annual Revenue:</strong> {companyInfo.revenue}</p>
        <p><strong>Contact Number:</strong> {account.phone}</p>
      </div>
      ...
      <div className="account-info">
        ...
        <p><strong>Start Date:</strong> {accountInfo.startDate}</p>
        <p><strong>Renewal Date:</strong> {accountInfo.renewalDate}</p>
      </div>
      ...
      <div className="attachment-section">
        <h2>Attachments</h2>
        <input className='filebtn' type="file" onChange={handleFileUpload} multiple />
        <ul className=''>
          {attachments.map((file, index) => (
            <li key={index} className='files'>
              {file.name}
              <button className='dltbtn' onClick={() => handleDeleteAttachment(index)}>Delete</button>
            </li>
          ))}
        </ul>
      </div>
