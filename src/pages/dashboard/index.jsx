    ...
    <div className="container">
      <div className="atenndance_inner">
        ...
        <div className="atenndance_right_box">
          ...
          <button onClick={toggleVisibility}><h2>Add Dashlets</h2></button>
      {isExpanded && (
        <div className="draggable-components">
          
          <ul>
            {DraggableComponents.map(component => (
              <li key={component.id} onClick={() => handleAddComponent(component.id)}>
                {component.name}
              </li>
            ))}
          </ul>
          ...
          {layout.map(item => (
            <div key={item.i} style={{ position: 'relative' }}>
              <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%' }}>
                {DraggableComponents.find(c => c.id === item.componentId)?.component}
              </div>
              {/* <div>
